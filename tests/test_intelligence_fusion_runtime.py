...
class ParallelModel:
    ...
    def complete(self, messages, tools, *, mission_id, run_id, turn_id, plan_version):
        ...
        if self.turn_count == 1:
            calls = tuple(ToolCallProposal.create(name, {}, mission_id=mission_id, run_id=run_id, turn_id=turn_id, action_id=f"a-{name}", tool_call_id=f"call-{name}", plan_version=plan_version) for name in ("status", "search"))
...
def test_native_runtime_parallel_calls_have_independent_results(tmp_path, monkeypatch):
    ...
    mission = runtime.create("collect", "collect", plan, completion_criteria=[{"criterion_id": "goal"}], request_id="req-fusion", authorization_context=make_test_authorization_context("req-fusion", tmp_path).to_dict())
    result = runtime.run_model_loop(mission.mission_id, ParallelModel(), tools=[{"name": "status"}, {"name": "search"}], max_turns=3)
