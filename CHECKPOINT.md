...
## SESSION 5 (continued, P1-B) — Offensive Capability Activation: Real Scoped TLS Observation (COMPLETE)

- Branch: security/b3-four-layer-intent
- Session resumed from the verified checkpoint state 6ce6b9215f88 (CI 1278/1 green; the only later commit 6c3b2971 was inspected and preserved — a CI status marker).
- The workspace had been reset between sessions again; byte-exactness discipline was re-applied: tools/registry.py rebuilt from its 21-commit patch chain (EVERY intermediate blob SHA verified; final blob bc1e731455...), agent/offensive_bridge.py from its 5-commit chain (blob 3dbee8a4cd...), and both OFFENSIVE_CAPABILITY docs (blobs fc76533162 / 8a1aacc2c9) — all verified BEFORE any edit.

### Milestone P1-B — scoped_tls_observation is REAL
- tools/registry.py: `_scoped_tls_observation` — a REAL bounded verifying TLS observation behind the same gate sequence as the probe/dns tools. The executed argument is the ScopeGuard's CANONICAL url; the handler derives host and port from that url and NOTHING else. SNI is NOT an input — it is ALWAYS the canonical host (a caller can never point the observation at a different virtual host). The handshake runs under ssl.create_default_context() (CERT_REQUIRED, check_hostname=True): verification semantics are FIXED, documented, and never relaxed to make an observation succeed. Handshake success and certificate validity are classified SEPARATELY (TLS_CERTIFICATE_VALID / TLS_CERTIFICATE_INVALID / TLS_CERTIFICATE_UNAVAILABLE). tls_version and cipher come from the ACTUALLY negotiated socket. SHA-256 fingerprint over the DER bytes. SANs sorted+deduped, capped at 32. Distinct deterministic fail-closed classifications: TLS_ARGUMENT_INVALID / TLS_CONNECTION_FAILED / TLS_TIMEOUT / TLS_HANDSHAKE_FAILED / TLS_CERTIFICATE_INVALID / TLS_RUNTIME_ERROR. Single TLS seam `_tls_connect`; no session/key material ever recorded. Explicit ports execute ONLY inside the scope asset's ports set (registry scope resolver rejects pre-network otherwise).
- agent/offensive_bridge.py: `ScopedTlsObservationAdapter` — same contract as the probe/dns adapters (bound typed scope context + bound typed Owner AuthorizationDecision; execution ONLY through tools.registry.execute with scope re-resolution and decision re-verification).
- tests/test_scoped_tls_observation.py (25 tests): handler bounds at the faked single TLS seam (negotiated values from the handshake result, fingerprint = SHA-256(DER), SAN determinism + cap, verification failure ≠ success, distinct fail-closed classifications, argument validation BEFORE the seam, no authority or secret/session keys, catalog/runtime disjointness) + LIVE bridge reachability (real typed Owner AuthorizationDecision + persisted scope snapshot + registry scope re-resolution; provenance-bearing evidence through the EXISTING chain) + dry-run + ten negatives (no scope snapshot, no owner decision, model-supplied arbitrary hostname, DNS-derived IP target, wrong port outside the asset's ports set, SAN scope-smuggling, wrong-tool decision rejected at proof derivation, tampered plan hash, unregistered tool) — every negative proven PRE-NETWORK (execute spy and TLS seam empty). No test performs a real network call.
- OBSERVATION ≠ AUTHORIZATION proven adversarially for certificate data: SAN entries and resolved endpoints can never widen scope.

### Commits and CI (this milestone)
- 4b1549c141b feat(tools): scoped_tls_observation handler + adapter (CI GREEN 1278/1)
- 3a41b43ea7ff test battery (CI GREEN 1303/1 — 25 new tests, first-run green)
- 4b831621948a docs: OFFENSIVE_CAPABILITY_MATRIX.md (scoped_tls_observation LIVE; port & service / technology fingerprinting still PLANNED, not claimed) + OFFENSIVE_CAPABILITY_CHECKPOINT.md P1-B milestone record

### P1-C status (HTTP observation)
HTTP observation is ALREADY LIVE as `scoped_http_probe` (real bounded HTTP observation since P0: one GET, redirects observed never followed, hard timeout, hard size cap, body hashed never returned, 9-test battery, live bridge test). It is NOT rebuilt. The next NEW capability is P1-D: port & service observation.

### Security invariants re-verified (evidence-backed)
- INV-OFF-1..7 unchanged and retested for the TLS family; single execution path; fail-closed before any network execution; dry-run executes nothing; catalog/runtime disjointness asserted; SNI always canonical; verification never relaxed; certificate data is observation, never authorization.

- NEXT_SESSION_FIRST_ACTION: verify diagnostics/ci-<this-checkpoint-commit-sha>.md is green at branch HEAD (expected 1303 passed / 1 skipped), then begin P1-D: port & service observation as a bounded, owner-authorized, scope-bound tool family (same pattern: real handler behind the registry scope firewall, single seam fakeable in tests, adapter reuse, adversarial + live bridge battery, CI green, checkpoint update), per docs/runtime/OFFENSIVE_CAPABILITY_CHECKPOINT.md NEXT_ACTION. Keep catalog/registry disjoint; do NOT touch main; no reset/rebase/squash/force-push; no scheduler/DAG/worker runtime.
- EXACT_RESUME_POINT: P0 + P1(dns_lookup) + P1-B(scoped_tls_observation) COMPLETE; HTTP observation LIVE as scoped_http_probe since P0. First incomplete step: P1-D port & service observation.
