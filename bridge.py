...
class Handler(BaseHTTPRequestHandler):
    server_version="CyberSentinelX/FINAL"
    ...
    def do_GET(self):
        if self.path=="/api/health":
            return self._send(200,{"ok":True,"service":"CyberSentinel X","version":"FINAL"})
        if self.path=="/":
    ...
    def do_POST(self):
        ...
        try:
            ...
            if n>32768: return self._send(413,{"ok":False,"error":"request_too_large"})
            data=json.loads(self.rfile.read(n) or b"{}")
            text=str(data.get("text","")).strip()
            if not text: return self._send(400,{"ok":False,"error":"text_required"})
            return self._send(200,handle(text,source="web",presented_token=self.headers.get("X-CyberSentinel-Token")))
        except Exception:
...
def main():
    ...
    print(f"CyberSentinel X FINAL: http://{BRIDGE_HOST}:{BRIDGE_PORT}")
