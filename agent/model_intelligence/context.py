...
class ContextAssembler:
    ...
    def build(self, mission: Any, *, conversation: Iterable[ConversationTurn] = (), tool_results: Iterable[dict[str, Any]] = (), tools: Iterable[dict[str, Any]] = ()) -> AssembledContext:
        ...
        durable_tool_results = tuple(tool_results)
        if durable_tool_results:
            ...
            messages = (system, state, *tuple(conversation), assistant_continuation, *tool_messages)
        else:
            messages = (system, state, *tuple(conversation))
