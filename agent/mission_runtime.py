...
class MissionRuntime:
    ...
    def run_slice(self, mission_id: str) -> Mission:
        ...
        from security.execution_plan_runtime import bind_execution_plan, gate_action_against_plan, legacy_plan_effective_tools, owner_budget_from_snapshot
        try:
            owner_budget = owner_budget_from_snapshot(mission)
            _effective_tools, canonical_plan = legacy_plan_effective_tools(owner_budget, mission.plan, request_id=mission.request_id)
        except Exception as exc:
            mission.error = f"canonical execution plan derivation failed: {exc}"
            mission.failures.append({"class": FailureClass.AUTHORIZATION.value, "reason": mission.error, "step_id": step.step_id})
            mission.emit(EventType.EXECUTION_REJECTED, step_id=step.step_id, data={"code": "PLAN_INVALID", "reason": mission.error})
            mission.transition(MissionStatus.AUTHORIZATION_BLOCKED, mission.error)
            return self.store.save(mission)
        if canonical_plan is None:
            mission.error = "no canonical ExecutionPlan can be derived from the current legacy plan; execution fails closed"
            mission.failures.append({"class": FailureClass.AUTHORIZATION.value, "reason": mission.error, "step_id": step.step_id})
            mission.emit(EventType.EXECUTION_REJECTED, step_id=step.step_id, data={"code": "PLAN_MISMATCH", "reason": mission.error})
            mission.transition(MissionStatus.AUTHORIZATION_BLOCKED, mission.error)
            return self.store.save(mission)
        ...
        if not gate_ok:
            mission.error = f"{gate_code}: {gate_reason}"
            mission.failures.append({"class": FailureClass.AUTHORIZATION.value, "reason": mission.error, "step_id": step.step_id})
            mission.emit(EventType.EXECUTION_REJECTED, step_id=step.step_id, data={"code": gate_code, "reason": gate_reason})
            mission.transition(MissionStatus.AUTHORIZATION_BLOCKED, mission.error)
            return self.store.save(mission)
