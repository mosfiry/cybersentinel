...
class MissionRuntime:
    ...
    def run_slice(self, mission_id: str) -> Mission:
        ...
        # with the validated legacy plan steps. The legacy Plan/PlanStep is
        ...
        from security.execution_plan_runtime import bind_execution_plan, gate_action_against_plan, legacy_plan_effective_tools, registered_owner_budget_from_snapshot
        from security.owner_budget import KNOWN_TOOLS
        try:
            owner_budget = registered_owner_budget_from_snapshot(mission)
            _effective_tools, canonical_plan = legacy_plan_effective_tools(owner_budget, mission.plan, request_id=mission.request_id)
        except Exception as exc:
        ...
        if canonical_plan is None:
            # B3-H3: the legacy plan cannot enter any canonical ExecutionPlan;
            # the malformed proposal fails closed without execution.
            return self._reject_action_outside_canonical_plan(mission, step, action_id, "PLAN_MISMATCH", "no canonical ExecutionPlan can be derived from the current legacy plan; execution fails closed", FailureClass.LOGIC)
