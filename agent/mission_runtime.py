...
class MissionRuntime:
                    ...
                    if not decision.allowed:
                        ...
                        try:
                            ...
                            if not proof_ok:
                                ...
                                self.store.save(mission)
                                raw = execute_tool(proposal.name, argument, authorization_decision=decision.decision, scope_context=mission.scope_snapshot, request_id=mission.request_id, mission_id=mission.mission_id, execution_proof=proof, execution_run_id=run_id)
    ...
    def _run_parallel_model_calls(self, mission: Mission, proposals: tuple[Any, ...], *, auth_context: Any, run_id: str, current_step: Any, progress: dict[str, Any], seen: set[str], plan: Any = None) -> None:
        ...
        def execute_one(item: tuple[Any, Any, Any, Any]) -> dict[str, Any]:
            ...
            try:
                return {"proof_ok": True, "proof_code": "", "proof_reason": proof_reason, "raw": dict(execute_tool(proposal.name, argument, authorization_decision=decision.decision, scope_context=mission.scope_snapshot, request_id=mission.request_id, mission_id=mission.mission_id, execution_proof=proof, execution_run_id=run_id) or {})}
            except Exception as exc:
    ...
    def _reject_action_outside_canonical_plan(self, mission: Mission, step: PlanStep, action_id: str, code: str, reason: str, failure_class: FailureClass) -> Mission:
        ...
        the retry budget stays enforced (LOGIC/COMPILATION/TEST_FAILURE ->
        bounded replan, AUTHORIZATION -> owner input, exhausted ->
        FAILED_RETRY_EXHAUSTED).
        """
        mission.error = f"{code}: {reason}"
        mission.failures.append({"class": failure_class.value, "reason": mission.error, "step_id": step.step_id, "action_id": action_id})
        mission.emit(EventType.EXECUTION_REJECTED, step_id=step.step_id, data={"code": code, "reason": reason, "action_id": action_id})
        mission.emit(EventType.FAILURE_DETECTED, step_id=step.step_id, data={"class": failure_class.value, "reason": mission.error, "code": code})
        mission.retry_count += 1
        action = self.recovery_policy.action_for(failure_class, mission.retry_count - 1)
        mission.emit(EventType.FAILURE_DIAGNOSED, step_id=step.step_id, data={"class": failure_class.value, "recovery": action.value})
        if action is RecoveryAction.OWNER_INPUT_REQUIRED:
            mission.transition(MissionStatus.OWNER_INPUT_REQUIRED, "canonical execution plan rejection requires owner input")
            ...
            mission.transition(MissionStatus.REPLANNING, "action rejected by the canonical execution plan")
            mission.emit(EventType.REPLAN_TRIGGERED, step_id=step.step_id, data={"reason": mission.error, "code": code})
            new_plan = self.replanner(mission, {"type": "execution_rejected", "success": False, "error": mission.error, "code": code, "action_id": action_id, "step_id": step.step_id})
            if new_plan.objective != mission.objective:
                ...
                return self.store.save(mission)
            mission.replan_history.append({"from_version": mission.plan.version, "to_version": new_plan.version, "reason": mission.error, "trigger": {"code": code, "action_id": action_id}})
            mission.plan_history.append({"version": new_plan.version, "fingerprint": new_plan.fingerprint, "reason": mission.error})
            mission.emit(EventType.PLAN_REVISED, data={"version": new_plan.version, "fingerprint": new_plan.fingerprint, "reason": mission.error})
            mission.plan = new_plan
            mission.progress.pop("execution_plan", None)  # B3-C5-A: replan drops the stale canonical binding; proofs bound to the previous canonical identity fail closed
    ...
    def run_slice(self, mission_id: str) -> Mission:
        ...
        # B3-C5-A/B (B3-H2, B3-H3): the legacy slice path executes only
        # through the canonical typed ExecutionPlan derived from the Owner
        # budget (reconstructed from the Owner-minted snapshot) intersected
        # with the validated current legacy step. The legacy Plan/PlanStep is
        # a compatibility representation: any step that is not part of the
        # derived canonical plan is deterministically rejected BEFORE the
        # executor is called (no Owner -> Legacy Plan -> Handler path).
        from security.execution_plan_runtime import bind_execution_plan, gate_action_against_plan, legacy_step_execution_plan, registered_owner_budget_from_snapshot
        from security.owner_budget import KNOWN_TOOLS
        try:
            owner_budget = registered_owner_budget_from_snapshot(mission)
        except Exception as exc:
            # Deterministic fail closed: the canonical authority view cannot be
            # derived, so nothing may execute (executor/handler never called).
            return self._reject_action_outside_canonical_plan(mission, step, action_id, "PLAN_INVALID", f"canonical execution plan derivation failed: {exc}", FailureClass.AUTHORIZATION)
        # B3-C5: canonical conversion of the CURRENT legacy step. The C3
        # ExecutionPlan is first-seen-unique per tool (INV-C4-H1-4), so a
        # whole-plan conversion collapses repeated same-tool steps into a
        # single action and every later identical step would fail the
        # strict identity gate forever. Per-step conversion keeps
        # acceptance identical (a step may execute iff its tool is
        # registered and inside the Owner budget: intersection at
        # derivation, B3-H4) while each repeated step derives its own fresh
        # canonical identity (INV-C4-H1-5: repetition is a new derivation,
        # never implicit plan multiplicity).
        canonical_plan = legacy_step_execution_plan(owner_budget, mission.plan, step, request_id=mission.request_id)
        if canonical_plan is None:
            # B3-H3: the legacy step cannot enter any canonical
            # ExecutionPlan (unregistered tool or empty Owner budget
            # intersection); the proposal fails closed without execution.
            rejection_class = FailureClass.LOGIC if str(step.action or "").strip() not in KNOWN_TOOLS else FailureClass.AUTHORIZATION
            return self._reject_action_outside_canonical_plan(mission, step, action_id, "PLAN_MISMATCH", "no canonical ExecutionPlan can be derived for the current legacy step; execution fails closed", rejection_class)
        stored_plan = mission.progress.get("execution_plan") or {}
        if not isinstance(stored_plan, dict) or stored_plan.get("plan_fingerprint") != canonical_plan.plan_fingerprint:
            # The bound plan identity is always the freshly derived canonical
            # plan: a tampered, stale, or forged stored binding is re-bound to
            # ground truth and can never widen what may execute.
            bind_execution_plan(mission, canonical_plan)
        step_arguments = dict(step.retry_policy or {}).get("arguments")
        if not isinstance(step_arguments, dict):
            step_arguments = {}
        gate_ok, gate_code, gate_reason = gate_action_against_plan(canonical_plan, action_id=step.step_id, tool_name=step.action, arguments=step_arguments)
        mission.emit(EventType.AUTHORIZATION_CHECKED, step_id=step.step_id, data={"allowed": gate_ok, "reason": gate_reason if not gate_ok else "canonical execution plan membership verified"})
        if not gate_ok:
            # B3-H3: an action that is not part of the derived canonical plan
            # is rejected BEFORE the executor is called (handler_calls == 0).
            # An unregistered tool is a malformed legacy proposal (LOGIC ->
            # bounded replan); a registered tool outside the Owner-derived
            # plan is an authorization failure (owner input required).
            rejection_class = FailureClass.LOGIC if str(step.action or "").strip() not in KNOWN_TOOLS else FailureClass.AUTHORIZATION
            return self._reject_action_outside_canonical_plan(mission, step, action_id, gate_code, gate_reason, rejection_class)
    ...
    def run_to_completion(self, mission_id: str, *, max_slices: int | None = None, heartbeat: Callable[[], None] | None = None) -> Mission:
        mission = self._load(mission_id)
        limit = max_slices or mission.max_iterations
        if not mission.is_terminal:
            # B3-C5-D: one fresh execution run identity per completion run.
            # MissionExecutionBoundary reads this live run id into every
            # slice-path proof, so a proof issued in one run can never
            # validate or execute in a later run of the same mission.
            mission.progress["execution_run_id"] = "run-" + hashlib.sha256((mission.mission_id + ":" + mission.request_id + ":" + uuid.uuid4().hex).encode()).hexdigest()[:20]
            self.store.save(mission)
